...
export async function generateQuestionsBatch(opts: {
  ...
  const prompt = [
    ...
    '',
    '【共通ルール】',
    '- そのトピックで実際に問われやすい、代表的・基本的な論点を選ぶ(重箱の隅や例外は避ける)。試験範囲を網羅する土台になるようにする。',
    '- ★最重要:正解は誰が見ても明確に1つだけにする。残り3つは「明らかに誤り」か「よくある間違い」にし、複数が正解に見える曖昧な問題・好みが分かれる問題は絶対に作らない。',
    '- 「最も適切」を問う場合も、不正解の選択肢には明確に劣る理由(失礼・順序が違う・事実誤りなど)を持たせる。',
